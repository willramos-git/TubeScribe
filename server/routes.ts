...
async function fetchTranscriptPython(videoId: string): Promise<any[]> {
  return new Promise((resolve, reject) => {
    ...
    const py = spawn(pythonCmd, ["server/fetch_transcript.py", videoId]);
    let output = "";
    let error = "";
      ...
      try {
        ...
        if (error.code === 'ENOENT') {
          return res.status(500).json({
            message: "Python 3 is required to fetch transcripts. Please install Python 3 and ensure it is accessible in your PATH."
          });
        }
        return res.status(500).json({
